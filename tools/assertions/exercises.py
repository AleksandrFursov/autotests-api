...
def assert_create_exercise_response(request: CreateExerciseRequestSchema,
    ...
    :param request: Исходный запрос на создание урока.
    :param response: Ответ API созданного урока.
...
def assert_get_exercise_response(
    ...
    """
    Проверяет, что ответ на получение задания соответствует ранее созданному заданию.
    """
...
@allure.step("Check get exercise with non-existent guid response")
def assert_exercise_not_found_response(actual: InternalErrorResponseSchema):
    """
    Проверяет, что ответ на несуществующее задание соответствует ожидаемой внутренней ошибке.

    :param actual: Ответ от API с внутренней ошибкой, которую необходимо проверить.
    :raises AssertionError: Если фактический ответ не соответствует ожидаемому.
    """
    logger.info("Check get exercise with non-existent guid response")
